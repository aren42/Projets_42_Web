...
<?php
	if ($_GET['action'] === "add") {
		...
		<?php
			...
			VALUES ('".uuid()."', '".htmlspecialchars($_POST['mail'])."', '".pass_hash($_POST['mail'], $_POST['password'])."', ".htmlspecialchars($_POST['rank']).", NOW())";
	...
	?>
		...
		<?php
			$sql = "SELECT uuid, mail, rank, create_at FROM users";
			$result = mysqli_query($connect, $sql);
